...
describe("walkable area", () => {
  test("rejects points outside the roaming bounds", () => {
    expect(isWalkable(WANDER_HALF + 1, 0)).toBe(false);
    expect(isWalkable(0, -WANDER_HALF - 1)).toBe(false);
  });
  ...
  test("rejects points inside a building footprint", () => {
    expect(BUILDING_BLOCKERS.length).toBeGreaterThan(0);
    for (const blocker of BUILDING_BLOCKERS) {
      expect(isWalkable(blocker.x, blocker.z)).toBe(false);
    }
  });
...
describe("wander targets", () => {
  test("returns walkable targets within hop range", () => {
    ...
    const villagers = createVillagers(VILLAGER_SEED, VILLAGER_COUNT);
    for (const villager of villagers) {
      for (let step = 0; step < 40; step += 1) {
        const target = pickWanderTarget(random, villager.x, villager.z);
        expect(isWalkable(target.x, target.z)).toBe(true);
...
describe("villager spawns", () => {
  test("spawns the requested count on walkable ground", () => {
    ...
    for (const villager of villagers) {
      expect(isWalkable(villager.x, villager.z)).toBe(true);
  ...
  test("ids are unique and spawns are deterministic", () => {
    const first = createVillagers(VILLAGER_SEED, VILLAGER_COUNT);
    const second = createVillagers(VILLAGER_SEED, VILLAGER_COUNT);
